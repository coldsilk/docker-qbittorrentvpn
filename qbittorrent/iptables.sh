...
while : ; do
	...
	if [[ ! -z "${tunnelstat}" ]]; then
		...
		if [[ ! -z "${VPN_CONF_SWITCH_OPENVPN_AT_START}" && "${VPN_CONF_SWITCH_OPENVPN_AT_START,,}" != "0" && "${VPN_CONF_SWITCH_OPENVPN_AT_START,,}" != "false" && "${VPN_CONF_SWITCH_OPENVPN_AT_START,,}" != "no" ]]; then
